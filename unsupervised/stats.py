...
class Stats(object):
    ...
    def write_to_disk_anomalies():
         ...
         query = "SELECT * FROM sensortgmeasure"

         measures = pd.read_sql(query , conn)
         
         for index, row in measures.iterrows():
              query = text("""INSERT INTO anomaliestg (idSensor, date, anomaly) VALUES(:idSensor, :date, :anomaly)""")    
              conn.execute(query, idSensor=row["sensortgId"], date=str(row["date"]), anomaly=0)
             ...
             if row['descricao']=='Percepcao':
                 ...
                 for date in dates_measures:
                     if date >= date_N and date <= date_event:
                          ...
                          conn.execute(query, anomaly=1, date=str(date))
    ...
    def update_interval_anomalies():
        ...
        for date_event in dates:
            date_N = Stats.date_N_days_ago(date_event, 2)
            query = "UPDATE anomaliestg SET anomaly=1 WHERE date >= %s and date <= %s"
