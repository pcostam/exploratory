...
def hyperparam_opt():
    ...
    dim_num_lstm_layers = Integer(low=0, high=100, name='num_lstm_layers')
    ...
    default_parameters = [2, 128, 1e-3, 1e-3]
...
def fitness(num_lstm_layers, batch_size, adam_decay, learning_rate):    
    print("fitness>>>")
    ...
    model = autoencoder_model(X_train_full, num_lstm_layers, adam_decay, learning_rate)
...
def autoencoder_model(X, num_lstm_layers, adam_decay, learning_rate):
    print("X", X)
    timesteps = X.shape[1]
...
def test_autoencoder():
    ...
    gp_result = gp_minimize(func=fitness,
                                ...
                                callback=es)
    param = gp_result.x
    num_lstm_layers = get_num_layers_lstm(param)
    batch_size = get_dim_batch_size(param)
    adam_decay = get_dim_adam_decay(dimensions)
    learning_rate = get_dim_learning_rate(dimension)
    print("number of lstm layers:", num_lstm_layers)
    print("batch size:", batch_size)
    print("adam decay:", adam_decay)
...
def get_dim_learning_rate(dimensions):
    return dimensions[3]
def get_dim_adam_decay(dimensions):
    return dimensions[2]
